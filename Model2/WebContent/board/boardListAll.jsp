...
// 		System.out.println(" M : 페이징 처리정보 저장 "); 
   ...
   <%
   	//[9/1]하단 페이징 처리
   	if(cnt != 0){ //게시글 수가 0이 아닐떄
   		
   		//이전 : 직전 페이지블럭의 첫번째 페이지 호출 
   		// 4 5 6 중에서 5번을 보고 있는데 startPage:4 pagaBlock:3 4-3은 1이 되는데 1페이지로 가는거 
   		// 9 10 11 중에서 11페이지 보고 있는데 이전 눌렀을때 몇 페이지? 9-3= 6이 나오고 6 7 8 페이지가 뜬다 => 블록을 옮겨버리는 개념 
   		if(startPage>pageBlock){    // [4] [5] [6] startPage : 4 / pageBlock 3개 
   			%>
